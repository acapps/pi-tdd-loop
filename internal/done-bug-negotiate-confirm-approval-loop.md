...
**Existing prompt pins (intentionally untouched):** `test/tools-negotiate-re-review.test.ts:85` pins the prompt **by reference** (`GP.promptNegotiateContractReReview("*_test.go")`) — passes unchanged after the sentence addition, listed here so the Tester knows it is deliberately not rewritten. `test/extension.test.ts:897` asserts `toContain("contract re-review")` — the new sentence does not remove that substring, so it passes unchanged; likewise intentionally untouched.
...
The truncate-with-ellipsis is load-bearing: `lastProposal` can be a full paragraph (the session's was ~500 chars); the reprompt must not re-inject the whole proposal every time. **Empty-proposal edge (pinned, acceptable):** with `lastProposal === ""`, line 2 renders `Current state: negotiate round ${round}.` — the trailing period belongs to the empty clause slot and is pinned as-is (moving it outside the conditional would change the non-empty line's punctuation for no gain). A test pins both renderings verbatim. **Call-site inventory (closed):** the function has exactly two call sites — `deliverReprompt` in `src/events/agent-settled/negotiate.ts:109` (production; passes `effect.round` / `effect.lastProposal ?? ""`), and two test assertions that pin the zero-arg call — `test/events/agent-settled/negotiate.test.ts:97` and `test/events/agent-settled/index.test.ts:477` (both rewritten, see Test Strategy). `promptNegotiateRepromptTester` is left unchanged (no equivalent ambiguity observed; a Tester reprompt always follows a proposal that exists), and `repromptTester` in `src/transitions.ts` does not populate the new effect fields.

### 4. Repeated-tool-call breaker (src/events/tool-call/index.ts, new)

- On each `tool_call` event: canonical key = `toolName + JSON.stringify(event.input, sortedKeys)` — one flat key, one counter per key. **`event.input` is the canonical form: the complete input object, recursively key-sorted** — `event.input` is a typed union (e.g. `WriteToolInput = { path, content }`), and keying on the whole object is what makes the key stable and distinct within a batch; narrowing to a single field (e.g. `path`) would merge distinct writes to the same file into one counter. The counter map is `Map<string, number>` keyed by that flat string (F5: the earlier draft's `Map<toolName, {key, count}>` was self-contradictory; a per-toolName slot cannot track multiple distinct arg sets).
- Counter map is per-turn. **Reset points (pinned set):** the counter map is cleared on `turn_start` and on `agent_settled` (both verified on the installed SDK's `ExtensionAPI.on` overloads: `turn_start` at types.d.ts:929, `agent_settled` at :926). `turn_start` fires for every fresh turn — including the ones the `/loop-*` commands start (`/loop`, `/loop-continue`, `/loop-restart`, `/loop-approve` in `index.ts:54-58` all end in `sendUserMessage(..., { triggerTurn: true })`) — so no per-command reset wiring is needed; the `agent_settled` reset is the belt for turns that settle without a following `turn_start`.
...
- Ownership: `src/events/tool-call/index.ts` owns the counter; the extension's event registration in `index.ts` wires `pi.on("tool_call", handler)` (the `ExtensionAPI.on` overload for `tool_call` is verified in the installed types). **The new file sits beside the existing `src/events/tool-call.ts` (path enforcement, spec 02) — that file is untouched.** The breaker is a separate module; `index.ts` gains a **second** `tool_call` registration (the existing `Ev.eventToolCall` at `index.ts:78` stays). Do not consolidate the two handlers — they answer different questions (path permission vs repetition) and have different input shapes.
...
- New tests for the breaker: identical call count 4 → not blocked; count 5 → `{ block: true, terminate: true }` + user message + debug entry; count 6 → still blocked (sticky); different args → counter does not advance; a `turn_start` event and an `agent_settled` event each clear the counter (the pinned reset set from §4); two writes to the same `path` with different `content` → distinct counters (whole-input-object key pin).
...
| 6 | needs-doc | Phase 1 pre-flight (2026-09-06), four pinning clarifications, no design change: (1) whole-input-object canonical key, (2) concrete reset set = `turn_start` + `agent_settled`, (3) empty-proposal dangling-period rendering pinned as acceptable, (4) the two existing contract-re-review prompt pins listed as intentionally untouched + the new `src/events/tool-call/index.ts` must not be consolidated with the existing `src/events/tool-call.ts` | accepted — pinned in Behavior §2/§3/§4, Inventory, and Test Strategy |
