...
- `src/gates.ts`: added `hasVitestCoverageProvider` (exported); `getTestCommand` gains `opts` parameter + typescript branch; `runGates` call site (line 44) passes `{ cwd }`. No imports added (`require.resolve` via `module.createRequire` or a `createRequire(import.meta.url)` helper — pin in the implementation; `module` is not in scope under ESM `"type": "module"`-style imports used here, so the spec pins: use `createRequire(import.meta.url).resolve` wrapped in try/catch, or `import { createRequire } from "node:module"` at the top of the file).
- `test/gate-signal-integrity.test.ts`: existing `describe("getTestCommand (B/C single invocation)")` block (lines 152-176) — the typescript case (lines 160-162) is kept for row 3 and gains sibling cases for rows 1–2.
...
- `bug-coverage-noop-ts.md` (repo root): see Scope — the TS row is closed by this unit; the file is NOT renamed to `done-` because the Java/jacoco row and the "fail with a named reason" semantics remain open.
...
- New tests (all mock the process/module boundary — no real vitest, no real `node_modules` mutation, per CLAUDE.md TEST SPEED RULE):
  - Row 1: `vi.mock` the `node:module` `createRequire` resolution to resolve → `getTestCommand("typescript", undefined, { cwd: X })` returns the `--coverage` string.
  - Row 2: same mock throwing (unresolvable) → returns `"npx vitest run"`; and the probe never throws out of `getTestCommand` (assert the call returns, not throws).
  - `hasVitestCoverageProvider` directly: resolvable → true; unresolvable → false; `require.resolve` throwing a non-Error → false.
  - `runGates` wiring (mock `node:child_process` `execFile` as the file already does): a typescript gate with the provider absent invokes `vitest` with args `["run"]` (no `--coverage`); with the provider present, `["run", "--coverage"]`.
