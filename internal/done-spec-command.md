...
- `normalizeOutDir(dir: string): string` — strips trailing slashes so the prompt and
  notify paths render as `<dir>/<slug>.md` (the default `internal/` would otherwise
  render `internal//<slug>.md`). Pinned: `"internal/"` → `"internal"`, `"internal"` →
  `"internal"`, `"backlog/"` → `"backlog"`, `""` → `""`. Applied to `--out` values and
  the default in `cmdSpec` before any path is built.
- `resolveGoal(goalArg: string, cwd: string): string | null` — `""` → `""` (verbatim, pinned); if `goalArg` starts with `@`
...
`cmdSpec` handler decision table (first-match-wins; `notify` is the pinned UI sink). Precedence: row 2 fires before the rubric is read, so `--slug "!!!"` with a missing rubric is row 2, not row 4 (no rubric warning, no turn):
...
| 5 | Happy path | `debug("spec: author turn for slug " + slug + " (loop phase: " + state.current.phase + ")")`; `pi.sendUserMessage(renderAuthorPrompt(...), { triggerTurn: true })`; `notify("Author: writing <outDir>/<slug>.md. Review it, then run /loop <outDir>/<slug>.md", "info")` where `<outDir>` is the `normalizeOutDir`-ed form (default renders `internal/add.md`, not `internal//add.md`); `ui.setStatus("loop", "spec author (one-shot — no loop state)")`; return |
...
`cmdSpec` reads `state.current.phase` only for the row-5 `debug` string above (the single verbatim form, with the phase suffix) — no second debug call exists.
...
- New tests, `test/spec-command.test.ts` (pure-function level; no process spawning, no
  ...
  - `slugSpecName` — delegates to `slugBugName`; pin `"   "` → `""`, `""` → `""`,
    `"Fix the Gate Runner"` → `"fix-the-gate-runner"`, `"a--b__c"` → `"a-b-c"`.
  - `normalizeOutDir` — pin `"internal/"` → `"internal"`, `"internal"` → `"internal"`,
    `"backlog/"` → `"backlog"`, `""` → `""`.
  - `resolveGoal` — `""` → `""` verbatim; goal string returned verbatim; `@file` form reads (mock
...
- New tests, `test/extension.test.ts` (`/spec command` describe block; command level, mocked
  ...
  - `--out` flag respected in the notify text and the prompt's `Output file:` line
    (both in the normalized single-slash form, e.g. `backlog/add.md`).
...
- `src/spec-command.ts` — added; final shape: imports, `AuthorPromptInput`, `slugSpecName`,
  `normalizeOutDir`, `resolveGoal`, `readRubric`, `renderAuthorPrompt`, `cmdSpec`. No other exports.
...
|---|----------|---------|-------------|
| 3 | needs-doc | `resolveGoal("")` not pinned in Interface | Accepted — pinned `""` → `""` verbatim in Interface + tests |
| 4 | needs-doc | `slugSpecName("")` not pinned in the test list | Accepted — `""` → `""` added to the `slugSpecName` test pins |
| 5 | nit | Decision-table rows 2 and 4 can both match (`--slug "!!!"` + missing rubric) | Accepted — precedence note added to the table intro: row 2 wins, handler returns before reading the rubric |
| 6 | nit | Two different row-5 `debug` strings (table vs. later paragraph) | Accepted — consolidated to the single verbatim string with the phase suffix |
| 7 | nit | Findings 1–2 (table-cell extraction artifacts), 7–10 (prose words, not functions) from the auto-finder | Rejected as noise — not spec defects |
| 8 | nit | Auto-finder reported a `readRubric(rubricPath)` signature | Rejected — misread; `readRubric(cwd)` was already correct in the spec |
| 9 | major | Phase B: default `--out internal/` + `"${outDir}/${slug}.md"` rendered `internal//add.md` (double slash) in the prompt and notify text; spec's row-5 pins assumed a single slash | Accepted — added `normalizeOutDir` (strips trailing slashes), applied to the default and `--out` values; Interface, row 5, Test Strategy, and Scope lines updated; 4 new unit pins + command-level pins corrected to the single-slash form |
| 10 | major | Phase B: the rubric-missing test could not fire row 4 from the repo root — `readSpec` falls back to the *process* cwd, where `docs/spec-authoring.md` exists | Accepted — test now `process.chdir`s into a fresh empty `mkdtemp` dir (restored in the same test); no source change needed, the fallback order is spec'd behavior ("same resolution order as readSpec") |
