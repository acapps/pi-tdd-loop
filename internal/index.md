...
| [bug-advance-effect-dual-path.md](bug-advance-effect-dual-path.md) | bug | **open** | Two appliers apply the same `advance` effect (agent-settled `applyAdvanceEffect` + tool-call `applyTransitionEffect`) and diverged — the tool-call path never sent the Phase B prompt, stalling the loop in two live sessions. Consolidate prompt delivery into one `deliverAdvancePrompt` helper; remove the dead `src/tools/` stubs from `main`. Overlaps `refactor-tools-split` on the dead-stub cleanup. |
