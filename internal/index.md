...
| [bug-negotiate-confirm-approval-loop.md](done-bug-negotiate-confirm-approval-loop.md) | bug | done (merged at `b9bd8db`; file moved to `done-` prefix) | — (observed in the refactor-state-model-divergence run, 2026-09-05; fixes a defect in bug-negotiate-drift's row 2) |
| [bug-loop-breaker-repetition-with-mutation.md](bug-loop-breaker-repetition-with-mutation.md) | bug | open | hard: bug-confirm-approval (merged); rewrites the key function that spec pinned |
| [bug-fragile-event-handler-selection.md](bug-fragile-event-handler-selection.md) | bug | open | — (test-only; coexists with the two-handler registrations that landed in `b9bd8db`) |
