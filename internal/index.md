...
| [fix-scope-check-baseline.md](fix-scope-check-baseline.md) | bug | **open** (2026-09-25, `fix-scope-check-baseline`) | Scope-check flags pre-existing untracked files as out-of-scope (session 01a0d6aa: `prompt-evolution/` + `prompt-forge/behavior-test.ts` blocked Phase B for 5 rounds). No baseline mechanism — the check can't distinguish files created by the Writer from files that existed before the spec. Fix: capture `git status --porcelain` at spec start as `state.baselineDirtyFiles`, filter it out at gate time. |
