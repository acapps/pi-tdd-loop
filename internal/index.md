...
| [fix-negotiated-resolution-dropped.md](fix-negotiated-resolution-dropped.md) | bug | **open** (2026-09-24, `fix-negotiated-resolution-dropped`) | Negotiate auto-advance (`deliverAdvance` → `promptNegotiateAutoAdvance`) carries no resolution, so a test-touching agreed resolution is silently dropped — the Writer is barred from editing `*.test.ts`, the gate goes green, the loop reports done (session 01a0d128). Persist the resolution, block the advance when it touches tests, refuse done until executed. |
