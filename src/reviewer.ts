 ...
 */
// JS keywords that a signature regex will match but that are not functions.
const NON_FUNCTION_NAMES = /^(function|if|for|return|const|var|let|interface|type|export|import)/;
...
function attachHeadingDescriptions(results: SpecFunc[], specText: string): void {
 ...
 */
type UniqueFinder = (key: string, finding: Omit<Finding, "id">) => void;

function makeUniqueFinder(findings: Finding[]): UniqueFinder {
  let id = 0;
  const seen = new Set<string>();
  return (key, finding) => {
    if (!seen.has(key)) { seen.add(key); findings.push({ ...finding, id: ++id }); }
  };
}
...
function findIssues(specText: string): Finding[] {
  ...
  const unique = makeUniqueFinder(findings);

  detectVaguePhrases(specText, unique);
  detectSubjectiveThresholds(specText, unique);
  detectMissingErrorHandling(specText, unique);
  detectConflictingStatements(specText, unique);
  for (const func of extractFunctions(specText)) {
    detectFunctionIssues(func, unique);
  }
  detectMissingTestStrategy(specText, unique);

  return findings;
}

const VAGUE_PHRASES = ["properly", "correctly", "obviously", "straightforward", "obvious", "complementary", "clean refactor"];

// 1. Vague design phrases
function detectVaguePhrases(specText: string, unique: UniqueFinder): void {
...
function detectSubjectiveThresholds(specText: string, unique: UniqueFinder): void {
...
function detectFunctionIssues(func: SpecFunc, unique: UniqueFinder): void {
...
function detectMissingTestStrategy(specText: string, unique: UniqueFinder): void {
  if (/test|assert|verify|expect/i.test(specText)) return;
...
function extractContext(text: string, word: string, radius = 40): string {
  ...
  const prefixed = start > 0 ? "…" + ctx : ctx;
  const suffixed = end < text.length ? prefixed + "…" : prefixed;
  return suffixed.replace(/\n/g, " ");
}
